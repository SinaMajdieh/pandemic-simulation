...
@export var config: SimulationConfig = SimulationConfig.new():
	set = set_simulation_config
...
func start(cfg: SimulationConfig = config) -> void:
	seed(cfg.seed_value)
	timer.reset()
	config = cfg
	zoom_pan.set_sizing(config.bounds, _get_bounds())
	cell_grid.custom_minimum_size = config.bounds
	cell_grid.set_spacing(
		config.infection_config.transmission_radius,
		config.infection_config.transmission_radius
	)

	agent_manager = AgentManager.new(config)

	if agent_renderer:
		remove_child(agent_renderer)
		agent_renderer.queue_free()

	agent_renderer = AgentRenderer.new(agent_manager, radius, segments, config.agent_count)
	add_child(agent_renderer)

	agent_manager.state_manager.seed_stage(
		config.initial_states[AgentStateManager.AgentState.EXPOSED] / float(config.agent_count),
		AgentStateManager.AgentState.EXPOSED
	)
	agent_manager.state_manager.seed_stage(
		config.initial_states[AgentStateManager.AgentState.INFECTIOUS] / float(config.agent_count),
...
func set_simulation_config(cfg: SimulationConfig = ) -> void:
	config = cfg
	update_simulation_from_config()

func update_simulation_from_config() -> void:
	if agent_manager:
		agent_manager.update_config(config)

func _on_simulation_container_resized() -> void:
	zoom_pan.set_sizing(config.bounds ,_get_bounds())
