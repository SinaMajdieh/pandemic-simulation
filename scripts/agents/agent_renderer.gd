...
func _init(radius_: float, segments_: int, agent_count_: int) -> void:
	radius = radius_
	segments = segments_
...
func update_from_manager(manager: AgentManager) -> void:
	...
	for i: int in range(manager.agent_count):
		agent_transform.origin = manager.movement_manager.positions[i]
		multimesh.set_instance_transform_2d(i, agent_transform)
		multimesh.set_instance_color(i, state_color_map[manager.state_manager.states[i]])
